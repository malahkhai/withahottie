...
Password recovery starts at `/forgot-password`. ReplyPass supplies the recovery callback explicitly, exchanges current PKCE links at `/auth/callback`, and accepts legacy Supabase recovery links that return credentials in the URL fragment. The reset page removes fragment credentials from the address bar before showing the new-password form. Keep the production and local callback URLs above in Supabase's redirect allowlist.
